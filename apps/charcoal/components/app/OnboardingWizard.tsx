...
const STEPS = [
  ...
  { key: "roster", title: "Roster", copy: "Add an address-backed provider so real settlement has a valid counterparty." },
  { key: "fund", title: "Fund", copy: "Capitalize the Space ledger before committing its first budget." },
  { key: "request", title: "Request", copy: "Create a real Request and assign the provider before work begins." },
...
export function OnboardingWizard({ embedded = false }: { embedded?: boolean } = {}) {
  ...
  const run = async (key: StepKey) => {
    ...
    try {
      ...
      if (key === "roster") {
        if (provider) {
          setResult(key, `${provider.displayName} is address-backed and ready for settlement.`);
        } else {
          if (!address.trim()) throw new Error("A provider wallet address is required for the Work step.");
          const participant = await createParticipant(spaceId, { kind, displayName, address: address.trim(), actorId });
          setResult(key, `Added ${participant.displayName} with a settlement address.`);
          await refresh();
        }
        finish(key);
        setActive("fund");
