...
export interface Receipt { receiptId: string; txHash: string; amount: string; asset: string; network: string; chainId: number; status: "SETTLED"; simulated: boolean; deliverableHash?: string }
...
export async function evaluateJob(spaceId: string, jobId: string, evaluatorId: string, approved: boolean, feedback?: string): Promise<{ job: Job; receipt?: Receipt; gaiaRefund?: string }> { return post(`/api/spaces/${q(spaceId)}/work/${q(jobId)}/evaluate`, { evaluatorId, approved, feedback }); }
