...
| **M9** | **Real-Time Onchain Event Indexer & Reconciliation** | `IN PROGRESS` | Restart-safe, idempotent `JobCreated` and state-only `JobFunded` indexer slices with persisted cursor; remaining lifecycle events, WebSocket transport, and reorg handling remain |
