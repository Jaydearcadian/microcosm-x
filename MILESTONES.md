...
| **M9** | **Real-Time Onchain Event Indexer & Reconciliation** | `IN PROGRESS` | Restart-safe, idempotent `JobCreated`, `JobFunded`, and `JobSubmitted` indexer slices with persisted cursor; remaining lifecycle events, WebSocket transport, and reorg handling remain |
