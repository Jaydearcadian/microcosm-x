...
| **SUITE-1** | CI / Quality | Full repository test suite passes green locally | `VERIFIED` | `make test` (177 tests passed, 0 failed) | 2026-09-25 |
| **AGENTIC-1** | Agent / E2E | Tool-using agent completes Request → Work → proof → evaluation → real testnet settlement and trace | `VERIFIED` | `node scripts/agentic-testnet-e2e.mjs`; 14 MCP calls; tx `0xbd1957ffcc4ce1d57b10cc785ae5c41ec329169ea2cda1ee9695fc9dd7c47556`; receipt status `1` at block `41894186` | 2026-09-25 |
...
| **M14-2** | Protocol Standards | Authenticated, session-bound x402 intents persist immutable normalized payment data, verify exact EIP-712 signatures, reject tamper/replay/expiry/wrong-signer/wrong-asset/wrong-chain inputs, and settle exactly once only through a compatible injected real-receipt adapter; unsupported settlement is explicit and side-effect free | `VERIFIED` | `node --test mcp/test/x402.test.js packages/server/test/x402.test.js` (M14-MCP-4, M14-SERVER-3 PASS) + `make test` (36 contract, 16 policy, 92 MCP, 14 server, 10 SDK tests passed) | 2026-09-25 |
| **M10-1** | Authority Attenuation | EIP-712 AuthorityDelegation envelopes bind exact base-unit caps, daily budget, counterparty subset, roles, asset, chain, expiry, nonce, and policy snapshot; parent signatures, tamper, replay, expiry, revocation, persistence, and MCP/REST/SDK parity are verified without delegated settlement | `VERIFIED` | `node --test packages/policy-engine/test/delegation.test.js mcp/test/delegation.test.js packages/server/test/delegation.test.js packages/sdk/test/delegation.test.js` (M10-1…M10-9 PASS) + `make test` (36 contract, 20 policy, 94 MCP, 16 server, 11 SDK tests passed; 177 total) | 2026-09-25 |
...
### 2026-09-25: M14 authenticated x402 intent lifecycle (M14-2)

* **Commands**: `node --test mcp/test/x402.test.js packages/server/test/x402.test.js`; `make test`; `node scripts/verify-proof-ledger.mjs`.
* **Output**: session identity spoofing was rejected; intent creation persisted the normalized selected accept, resource, requester, expiry, digest, and activity; exact EIP-712 signing and tamper, duplicate signature, expiry, wrong signer, asset, and chain controls passed; unsupported settlement returned `UNSUPPORTED_SETTLEMENT` with a pending intent; the injected adapter was called once, returned a real transaction-hash/receipt shape, and a second settlement was rejected. The full gate passed 36 contract, 16 policy, 92 MCP, 14 server, and 10 SDK tests with 0 failures.
* **Status**: `VERIFIED` for the additive M14 intent lifecycle. No real facilitator is configured in production; unsupported settlement remains explicit and no simulated settlement path exists.

### 2026-09-25: M10 first authority attenuation slice (M10-1)
* **Commands**: `node --test packages/policy-engine/test/delegation.test.js mcp/test/delegation.test.js packages/server/test/delegation.test.js packages/sdk/test/delegation.test.js`; `make test`; `node scripts/verify-proof-ledger.mjs`.
* **Output**: EIP-712 typed-data binding and signature verification passed; 2-of-3 counterparty subset, exact six-decimal cap and daily-budget attenuation, role non-escalation, asset/chain/expiry/address/limit rejection, tamper, duplicate nonce/delegation, wrong signer, expiry, revocation, persistence, and MCP/REST/SDK parity passed. The full gate passed 36 contract, 20 policy, 94 MCP, 16 server, and 11 SDK tests with 0 failures.
* **Status**: `VERIFIED` for additive M10 authority envelopes. Delegated settlement and `/payments` semantics remain unchanged and untested as delegated behavior.
...
* **Status**: `VERIFIED` for state-only resolution and attested evidence. WebSocket subscription and reorg handling remain open.

### 2026-09-25: Final metadata and evidence state-only reconciliation (M9-8)
* **Commands**: `node --test mcp/test/indexer.test.js`; `make test`.
* **Output**: focused indexer tests passed 56/56 and the full repository gate passed 148/148 (36 contract, 13 policy, 81 MCP, 9 server, 9 SDK). `ProviderSet`, `BudgetSet`, `AdjudicatorSet`, and `RubricSet` applied only to Open indexed jobs; `EvidenceAttached` applied only to Funded/Submitted jobs, recorded the observed deliverable hash and source log, preserved `evidenceUri: null`, and left status unchanged. Canonical same-block ordering, invalid ordering, conflicts, identical replay no-ops, Space balance/escrow/receipt no-side-effects, and restart persistence passed.
* **Status**: `VERIFIED` for the final state-only M9 metadata/evidence slice. WebSocket subscription and reorg handling remain open.

### 2026-09-25: M9 transport, reconciliation, and reorg reliability (M9-9/M9-10)
* **Commands**: `node --test mcp/test/indexer.test.js`; `npm --workspace=@microcosm/mcp-server test`; `make test`; `node scripts/verify-proof-ledger.mjs`.
* **Output**: focused indexer tests passed 61/61, the full MCP suite passed 86/86, and the full repository gate passed 153/153 (36 contract, 13 policy, 86 MCP, 9 server, 9 SDK). Tests verified HTTP/WebSocket/injected client selection, explicit persisted `RECONCILING`/`RECONCILED` state, sanitized failure visibility, recovery, serialized polling with clean stop, canonical safe-checkpoint restoration after restart, orphaned event rewind, and legacy hashless cursor compatibility.
* **Status**: `VERIFIED`. The implemented worker polls over the selected transport; it does not claim push subscription.
...
* **Transaction**: `0xbd1957ffcc4ce1d57b10cc785ae5c41ec329169ea2cda1ee9695fc9dd7c47556`; independent `cast receipt` confirmed `status 1` at block `41894186` on chain `1952`.
